...
public class StaticDataRepository {
     ...
     *
     * @param attacks
     *            a map of attack IDs to Attack objects representing the default attacks to be saved
     *            in the database.
     */
    ...
    private void saveAttack(Attack attack) throws SQLException {
        try(PreparedStatement psAttack = this.dbConnection
                .prepareStatement(this.dbRepository.getSql("SaveAttack"))) {
    ...
    private void saveAttackEffects(Attack attack) throws SQLException {
        ...
        try(PreparedStatement psEffect = this.dbConnection
                .prepareStatement(this.dbRepository.getSql("SaveEffect"))) {
     ...
     *
     * @param bugemons
     *            a list of Bugemon objects representing the default Bugemons to be saved in the
     *            database. Each Bugemon should have its attacks already defined and linked by their
     *            IDs, as this method assumes that the attacks have been saved beforehand.
     */
    private void saveGameDataBugemon(List<Bugemon> bugemons) {
        try(PreparedStatement ps = this.dbConnection
                .prepareStatement(this.dbRepository.getSql("SaveBugemon"))) {
        ...
        try(PreparedStatement ps = this.dbConnection
            ...
            while (rs.next()) {
                ...
                builder.id(rs.getString(DatabaseColumns.COL_ID))
                        .name(rs.getString(DatabaseColumns.COL_NAME)).type(type)
                        ...
                        .hp(rs.getInt(DatabaseColumns.COL_BASE_MAX_HP)).addAttack(attack1)
                        .addAttack(attack2).addAttack(attack3)
     ...
     *
     * @param attackId
     *            the ID of the attack to be retrieved from the database. This ID should correspond
     *            to an attack that has been previously saved in the database, either as part of the
     *            default game data or through other means.
    ...
    public Attack getAttackById(String attackId) {
        try(PreparedStatement ps = this.dbConnection
                .prepareStatement(this.dbRepository.getSql("GetAttackById"))) {
     ...
     *
     * @param attackId
     *            the ID of the attack whose effects are to be retrieved from the database. This ID
     *            should correspond to an attack that has been previously saved in the database,
     *            either as part of the default game data or through other means.
            ...
            while (rs.next()) {
                switch (rs.getString(DatabaseColumns.COL_TYPE)) {
                    case "EffectStatModifier" :
                        EffectTarget target = DatabaseHelper.getEnumOrNull(rs,
                                DatabaseColumns.COL_TARGET, EffectTarget.class);
