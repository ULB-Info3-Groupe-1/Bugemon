...
/**
 * Holds the state of an ongoing level-up sequence. The controller populates it via {@link #start(List)} and advances it
 * via {@link #advance()}. The view reads {@link #getCurrent()} in its {@code refresh()} — it never receives data pushed
 * by the controller.
 */
