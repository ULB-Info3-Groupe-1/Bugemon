 ...
 *
 * <p>
 * Provides the shared {@link #handleCombatResult(Trainer, Trainer)} method that distributes XP and
 * navigates to the correct outcome screen. Concrete subclasses drive the combat loop and call
 * {@code view.refresh()} after each model mutation; they never push data into the view directly.
 * </p>
 *
 * @param <V>
...
public abstract class CombatController<V extends CombatView> extends Controller<V> {
    ...
    /**
     * Plays the attack animations contained in a turn result, then invokes {@code onFinished}. If
     * the turn has no attacks, the callback is executed immediately.
     *
     * @param result
    ...
    protected AutoTrainer createRandomOpponent(int playerTeamSize) {
        return new AutoTrainer(CombatService
                .createRandomTeam(this.playerService.getAllDefaultBugemons(), playerTeamSize));
    }

    /**
     * Resolves the end of a combat session by distributing XP on victory and navigating to the
     * appropriate outcome screen.
     *
     * @param winner
     *            the winning trainer, used to determine if the player won or lost.
     * @param playerTrainer
     *            the player's trainer, used to determine if the player won or lost and to
