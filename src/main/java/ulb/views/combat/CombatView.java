 ...
 *
 * <p>
 * {@code CombatView} loads the shared {@code Combat.fxml} layout and exposes the FXML-injected
 * components that are common to every combat mode: Bugemon info panels, sprite images, the action
 * menu container, the team switcher pane and the dialog zone.
 * </p>
 *
 * <p>
 * Concrete subclasses ({@link AutomaticCombatView}, {@link ManualCombatView}) must implement
 * {@link #initCombatMode()} to configure which UI regions are visible and how they behave for their
 * specific mode.
 * </p>
 *
 * <p>
 * The controller layer interacts with the combat UI exclusively through the public methods of this
 * class, keeping all JavaFX node manipulation out of the controller.
 * </p>
 *
...
public abstract class CombatView extends View {
    ...
    /**
     * Info panel (name, type, HP bar) for the player's active Bugemon, displayed on the player's
     * side of the combat screen.
     */
    @FXML
    protected BugemonInfoView bugemonTrainerInfo;

    /**
     * Info panel (name, type, HP bar) for the opponent's active Bugemon, displayed on the
     * opponent's side of the combat screen.
     */
    @FXML
    ...
    /**
     * Container for the action menu components (main menu, attack menu, …). Subclasses populate
     * this container via their own menu components.
     */
    @FXML
    protected ActionMenuView actionMenuView;

    /**
     * Overlay banner used to display turn feedback messages such as attack effectiveness or KO
     * notifications. Toggled visible/invisible by {@link #showDialog(String, String)} and
     * {@link #hideDialog()}.
     */
    @FXML
     ...
     *
     * <p>
     * Subclass constructors must call {@code super()} and then invoke {@link #initCombatMode()} to
     * finalise their mode-specific UI setup.
     * </p>
     *
     * @throws IOException
    ...
    protected CombatView() throws IOException {
        ...
        this.attackAnimationView = new CombatAnimationView(this.bugemonTrainerImage,
                this.bugemonOpponentImage);
    }
     ...
     *
     * <p>
     * Typical implementations show or hide regions that are irrelevant for their mode (e.g.
     * {@link AutomaticCombatView} hides the action menu and team pane, while
     * {@link ManualCombatView} shows the main action menu). This method is called once by the
     * subclass constructor after the FXML components have been injected.
     * </p>
     */
     ...
     *
     * <p>
     * Typical uses include showing attack-effectiveness feedback ("ATTAQUE EFFICACE !") or KO
     * announcements. The dialog zone remains visible until {@link #hideDialog()} is called.
     * </p>
     *
     * @param dialog
     *            the main message to display; must not be {@code null}.
     * @param additionalInfo
     *            a secondary line of text, or {@code null} if no additional information should be
     *            shown.
     */
    ...
    /**
     * Builds and displays the turn-summary dialog from the two attack results of the last resolved
     * turn. Only the attacks that actually happened are shown; the second result is absent when one
    ...
    /**
     * Hides the dialog zone, removing it from the layout flow so that it does not occupy space when
     * empty.
     */
    ...
    /**
     * Updates the player-side info panel and sprite to reflect the given Bugemon's current state
     * (name, type, HP).
     *
     * <p>
     * Should be called by the controller at the start of a combat session and after every turn in
     * which the player's active Bugemon may have changed or taken damage.
     * </p>
     *
     * @param trainerBugemon
     *            a {@link BugemonDTO} snapshot of the player's currently active Bugemon; must not
     *            be {@code null}.
     */
    protected void updateTrainerBugemon(BugemonDTO trainerBugemon) {
        this.bugemonTrainerInfo.setBugemonInfo(trainerBugemon);
        this.bugemonTrainerImage
                .setImage(new Image(trainerBugemon.getSpriteURL(), 256, 256, true, false));
        this.makeTrainerBugemonReappear();
    }

    /**
     * Updates the opponent-side info panel and sprite to reflect the given Bugemon's current state
     * (name, type, HP).
     *
     * @param opponentBugemon
     *            a {@link BugemonDTO} snapshot of the opponent's currently active Bugemon; must not
     *            be {@code null}.
     */
    protected void updateOpponentBugemon(BugemonDTO opponentBugemon) {
        this.bugemonOpponentInfo.setBugemonInfo(opponentBugemon);
        this.bugemonOpponentImage
                .setImage(new Image(opponentBugemon.getSpriteURL(), 256, 256, true, false));
    ...
    /**
     * Plays a lunge animation on the player's sprite (slide toward the opponent then return), then
     * invokes {@code onFinished} on the JavaFX thread.
     *
     * @param onFinished
    ...
    /**
     * Plays a lunge animation on the opponent's sprite (slide toward the player then return), then
     * invokes {@code onFinished} on the JavaFX thread.
     *
     * @param onFinished
     *            callback executed once the animation completes; must not be {@code null}.
